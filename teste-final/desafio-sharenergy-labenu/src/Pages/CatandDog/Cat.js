...
import { Img,Div,Input,Button,Cats} from "./CatsStyled"
...
const Cat = ()=>{
    ...
    const navigate = useNavigate()
        ...
        const statusCode = {
         ...
         const catsImg=()=>{
            if(statusCode[catList]){
                return <Img src={`https://http.cat/${catList}`}></Img>
              }else{
    ...
    return (
        ...
        <Input placeholder="Digite um statusCode" value={catList} onChange={((event)=> setCatList(event.target.value))}></Input> 
        {catsImg()}
        <Button onClick={()=>{navigate("/dog")}}>Cachorros</Button>
